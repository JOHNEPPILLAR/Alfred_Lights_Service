...
exports.processData = async (sensor) => {
  ...
  try {
    ...
    if (motion && lowLight) {
      ...
      const params = { lightGroupNumber: 8 };
      ...
      if (!lightstate.state.attributes.any_on) {
        ...
        try {
          const SQL = 'SELECT start_time, end_time, light_group_number, light_action, brightness, turn_off, scene FROM sensor_schedules WHERE active AND sensor_id = 2';
          serviceHelper.log('trace', 'Connect to data store connection pool');
          const dbConnection = await serviceHelper.connectToDB('lights');
          serviceHelper.log('trace', 'Get list of active services');
