  ...
  try {
    ...
    if (motion && lowLight) {
      ...
      if (!lightstate.any_on) {
        ...
        try {
          const SQL =
            'SELECT start_time, end_time, light_group_number, light_action, brightness, turn_off, scene FROM sensor_settings WHERE active AND sensor_id = 2';
          serviceHelper.log('trace', 'Connect to data store connection pool');
            ...
            if (
              ...
              switch (lightInfo.turn_off) {
                ...
                case 'FALSE':
                  ...
                  break;
                default:
                  lightsOffScheduleActive = await checkOffTimerIsActive(
                    lightInfo.turn_off,
                  );
              }
              serviceHelper.log(
                'trace',
                `Lights off scheduler active: ${lightsOffScheduleActive}`,
              );
