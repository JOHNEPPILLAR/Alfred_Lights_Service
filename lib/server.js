...
server.use((req, res, next) => {
  if (process.env.Debug === 'true') serviceHelper.log('trace', 'server', req.url);
...
server.on('uncaughtException', (req, res, route, err) => {
  ...
  serviceHelper.sendResponse(res, 'server', err);
});
...
async function cleanExit() {
  serviceHelper.log('warn', 'server', 'Service stopping');
  serviceHelper.log('warn', 'server', 'Closing the data store pools');
  await global.lightsDataClient.end();
  await global.schedulesDataClient.end();
  serviceHelper.log('warn', 'server', 'Close rest server');
  server.close(() => { // Ensure rest server is stopped
    serviceHelper.log('warn', 'server', 'Exit the app');
...
global.lightsDataClient.on('error', (err) => {
  serviceHelper.log('error', 'server', 'Lights data store: Unexpected error on idle client');
  serviceHelper.log('error', 'server', err.message);
  cleanExit();
});

global.schedulesDataClient.on('error', (err) => {
  serviceHelper.log('error', 'server', 'Schedules data store: Unexpected error on idle client');
  serviceHelper.log('error', 'server', err.message);
  cleanExit();
});
