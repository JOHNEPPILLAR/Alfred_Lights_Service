...
exports.processData = async (sensor) => {
  ...
  try {
    ...
    if (motion && lowLight) {
      ...
      const baseParams = { lightGroupNumber: 7 };
      ...
      if (!lightstate.state.attributes.any_on) {
        ...
        try {
          const SQL = 'SELECT start_time, end_time, light_group_number, brightness, turn_off, scene FROM sensor_schedules WHERE active AND sensor_id = 1';
          serviceHelper.log(
            'trace',
            'Connect to data store connection pool',
          );
