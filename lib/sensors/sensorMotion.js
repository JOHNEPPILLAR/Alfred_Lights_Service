...
async function processData(sensor, lightGroupNumber) {
  ...
  if (motion && lowLight) {
    this.logger.trace(
      `${this._traceStack()} - ${this.getLightGroupName.call(
        this,
        lightGroupNumber,
    ...
    try {
      // Turn on light group
      this.logger.trace(`${this._traceStack()} - Construct lights on api call`);
      ...
      if (results[0].turnOff) {
        ...
        this.logger.debug(
          `${this._traceStack()} - Setting ${this.getLightGroupName.call(
            this,
            lightGroupNumber,
