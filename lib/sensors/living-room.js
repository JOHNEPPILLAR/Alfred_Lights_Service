...
async function checkOffTimerIsActive(timerID) {
  serviceHelper.log(
    'trace',
    `Getting data for timer ${timerID}`,
  );
  let active = true;
  try {
...
exports.processData = async (sensor) => {
  ...
  try {
    ...
    if (motion && lowLight) {
      ...
      const baseParams = { lightGroupNumber: 8 };
      const lightstate = await lightGroupHelper.lightGroupState({ params: baseParams });
      ...
      if (!lightstate.state.attributes.any_on) {
        ...
        try {
          const SQL = 'SELECT start_time, end_time, light_group_number, brightness, turn_off, scene FROM sensor_schedules WHERE active AND sensor_id = 2';
          serviceHelper.log(
            'trace',
            'Connect to data store connection pool',
          );
          ...
          results.rows.map(async (lightInfo) => {
            if (
              ...
              serviceHelper.log(
                'trace',
                `Lights off scheduler active: ${turnOffLightTimer}`,
              );
