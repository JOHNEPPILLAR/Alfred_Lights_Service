...
const debug = require('debug')('Lights:Hue');

async function updateCaches() {
  // Check cache backoff state
  if (this.cacheTempIntival > this.cacheIntival) {
    ...
    debug(`Setting timer to ${this.cacheTempIntival / 1000} seconds`);
  }
  ...
  try {
    ...
    if (this.cacheTempIntival === this.cacheIntival) {
      this.cacheTempIntival *= 3; // Backoff polling intival
      debug(
        `Setting cache backoff timer to ${
