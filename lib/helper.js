...
async function callAlfredServicePut(apiURL, body, retry) {
  ...
  try {
    ...
    if (!apiURL.match(process.env.LogService)) { // Do not get in endless loop
    ...
    if (retryableCodes.indexOf(err.cause.code) >= 0 &&
 ...
 */
async function callAlfredServiceGet(apiURL, retry) {
  const retryableCodes = [
    'ECONNRESET',
    'ETIMEDOUT',
    'ECONNREFUSED',
...
function log(type, functionName, message) {
  if (process.env.Environment === 'dev') {
    ...
    const logFriendlyMessage = JSON.stringify(message);
    const body = {
      type, service: process.env.ServiceName, functionName, logFriendlyMessage,
    };
